---
title: Pobierz odpowiedzi na certyfikat | Microsoft Docs
description: ''
keywords: ''
author: billmath
ms.author: billmath
manager: daveba
ms.date: 09/26/2017
ms.topic: reference
ms.prod: microsoft-identity-manager
ms.assetid: b0559e7b-eaff-499b-8bcf-c6cdf5a89545
audience: developer
ms.reviewer: mwahl
ms.suite: ems
ms.openlocfilehash: 75c92bd31cda28c5eaef7d16832c819e27737525
ms.sourcegitcommit: 7e8c3b85dd3c3965de9cb407daf74521e4cc5515
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 03/10/2020
ms.locfileid: "92760679"
---
# <a name="get-certificate-responses"></a>Pobierz odpowiedzi na certyfikat
Przesyła zestaw żądań certyfikatów do urzędu certyfikacji w celu przetworzenia.

>[!IMPORTANT]
>Liczba odpowiedzi certyfikatu może przekroczyć liczbę żądań. Niektóre szablony profilów można skonfigurować w taki sposób, aby generować klucze/żądania na serwerze lub mieć certyfikaty zewnętrzne.

>[!NOTE]
>Adresy URL w tym artykule odnoszą się do nazwy hosta wybranej podczas wdrażania interfejsu API, takich jak `https://api.contoso.com` .

## <a name="request"></a>Żądanie

Metoda  |Adres URL żądania  
---------|---------
POST     |/CertificateManagement/api/v1.0/requests/{id}/certificatedata

### <a name="url-parameters"></a>Parametry URL

Parametr | Opis
--------|--------------
identyfikator| Wymagane. Identyfikator GUID odpowiadający szablonowi profilu, z którego mają zostać wyodrębnione zasady.

### <a name="request-headers"></a>Nagłówki żądań
W przypadku typowych nagłówków żądań zapoznaj się z [nagłówkami żądanie HTTP i odpowiedzi](certificate-management-rest-api-service-details.md#http-request-and-response-headers) w obszarze *Szczegóły usługi interfejsu API REST* .

### <a name="request-body"></a>Treść żądania
Treść żądania zawiera następujące właściwości:

Właściwość | Opis
---------|-----------
pfxpassword | Hasło, które jest używane do ochrony wszelkich wyników wymiany informacji osobistych (PFX), które są zwracane z wywołania.
certificaterequests | Lista żądań certyfikatów, które są generowane przez klienta programu. Żądania muszą być generowane przy użyciu parametrów, które są określone w obiekcie CertificateRequestGenerationOptions. Aby uzyskać więcej informacji, zobacz sekcję [pobieranie opcji generacji żądania certyfikatu](get-certificate-request-generation-options.md).
komentarz | Ciąg zawierający dodatkowe komentarze, które są rejestrowane w historii zdarzeń operacji.


## <a name="response"></a>Reakcja
W tej sekcji opisano odpowiedź.

### <a name="response-codes"></a>Kody odpowiedzi

Kod  |Opis  
---------|---------
200 | OK
403 | Forbidden
500 | Błąd wewnętrzny

### <a name="response-headers"></a>Nagłówki odpowiedzi
W przypadku typowych nagłówków odpowiedzi zobacz [nagłówki żądań i odpowiedzi HTTP](certificate-management-rest-api-service-details.md#http-request-and-response-headers) w *szczegółach usługi interfejsu API REST* .

### <a name="response-body"></a>Treść odpowiedzi
Po pomyślnym zwraca listę obiektów [Microsoft. CLM. Shared. Certificates. Certificate](https://msdn.microsoft.com/library/microsoft.clm.shared.certificates.certificate.aspx) z następującymi właściwościami:

Właściwość| Opis
------|-------------
CertificateId | Identyfikator certyfikatu.
Błąd | Wystąpił błąd podczas przetwarzania żądania certyfikatu.
Flagi | Flagi opisujące certyfikat.
Isexternal | Wartość logiczna wskazująca, czy certyfikat jest certyfikatem zewnętrznym.
isKeyHistory | Wartość logiczna wskazująca, czy certyfikat jest certyfikatem historii klucza.
isPfx | Wartość logiczna wskazująca, czy odpowiedź żądania certyfikatu jest w formacie PFX.
isPkcs7 | Wartość logiczna wskazująca, czy odpowiedź żądania certyfikatu ma format PKCS # 7.
isServerGenerated | Wartość logiczna wskazująca, czy certyfikat został wygenerowany przez serwer.
PFX | Obiekt BLOB certyfikatu PFX.
Pkcs7 | Obiekt BLOB certyfikatu PKCS # 7.
TemplateCommonName | Nazwa pospolita szablonu certyfikatu dla certyfikatu.
UseLocalMachineStore | Wartość logiczna wskazująca, czy certyfikat jest przeznaczony dla magazynu certyfikatów komputera lokalnego.

## <a name="example"></a>Przykład
Ta sekcja zawiera przykład przesyłania zestawu żądań certyfikatów do urzędu certyfikacji w celu przetworzenia.

### <a name="example-request"></a>Przykład: żądanie

```
POST /CertificateManagement/api/v1.0/requests/C6BAD97C-F97F-4920-8947-BE980C98C6B5/certificatedata HTTP/1.1

{
    "pfxpassword":"6b75162c-b3b4-4fc1-a653-f12dae98f0c3",
    "certificaterequests":[
        "\r\n"
    ]
}
```

### <a name="example-response"></a>Przykład: odpowiedź

```
HTTP/1.1 200 OK

[
    {
        "TemplateCommonName":"ContosoVirtualSmartCardLimitedRelease-KSP",
        "CertificateId":0,
        "Flags":65536,
        "UseLocalMachineStore":false,
        "Error":null,
        "Pkcs7":"\r\n",
        "Pfx":null,
        "IsExternal":false,
        "isServerGenerated":false,
        "isPfx":false,
        "isPkcs7":true,
        "isKeyHistory":false
    }
]
```

## <a name="see-also"></a>Zobacz także

- [Microsoft.Clm.Provision.ExecuteOperations. rejestrowania — Metoda](https://msdn.microsoft.com/library/microsoft.clm.provision.executeoperations.enroll.aspx)
- [Microsoft. CLM. Shared. Certificates. Certificate — Klasa](https://msdn.microsoft.com/library/microsoft.clm.shared.certificates.certificate.aspx)
